...
/**
 * App cliente actual (GestionPlus / NexProv) en esta petición.
 * Default: gestion — no cambia el comportamiento si el header no viene.
 */
final class ClientApp
{
    ...
    public static function applyOauthState(?string $state): void
    {
        if (! is_string($state) || ! str_starts_with($state, 'app.')) {
            return;
        }

        self::setCurrent(substr($state, 4));
    }
}
